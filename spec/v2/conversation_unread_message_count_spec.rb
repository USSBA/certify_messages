  ...
  RSpec.describe "V2 CertifyMessages", type: :feature do
    ...
    describe "unread_message_counts operations", :vcr do
      context "when getting message_counts" do
        let(:application_ids) { [V2::MessageSpecHelper.mock_user1_uuid, V2::MessageSpecHelper.mock_user2_uuid].join(',') }
        let(:recipient_uuid) { V2::MessageSpecHelper.mock_user1_uuid }
        let(:mock) { MessageSpecHelper.mock_unread_message_counts(application_ids, recipient_uuid) }
        let(:message_counts) { CertifyMessages::Conversation.unread_message_counts({application_ids: application_ids, recipient_uuid: recipient_uuid}) }
